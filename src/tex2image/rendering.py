...
def _ensure_installed(binary: str) -> None:
    if not shutil.which(binary):
        raise MissingBinaryException(
            f"This function requires `{binary}`. Please ensure it is installed."
        )
...
\end{document}
"""
"""The default template for [`latex_to_png`][tex2image.rendering.render_latex_to_png].

This is passed to `DEFAULT_TEMPLATE.format(snippet=latex_snippet)` in
[`latex_to_png`][tex2image.rendering.render_latex_to_png].
"""
